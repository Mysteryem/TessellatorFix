...
//        classToMethodMap.put("net.minecraft.entity.Entity", Transformer::patchEntity);
        classToMethodMap.put("net.minecraft.entity.item.EntityBoat", Transformer::logEntityBoatGetDamageTaken);
        classToMethodMap.put("net.minecraft.network.datasync.EntityDataManager", Transformer::patchEntityDataManager);
    }
...
//                                    "uk/co/mysterymayhem/debugmod/Hooks",
//                                    "debugHook",
...
//    }
    ...
    private static byte[] patchEntityDataManager(byte[] bytes) {
                ...
                for (ListIterator<AbstractInsnNode> iterator = methodNode.instructions.iterator(); iterator.hasNext(); ) {
                    ...
                    if (!(next instanceof LabelNode) && !(next instanceof LineNumberNode)) {
                        ...
                        iterator.add(new MethodInsnNode(
                                Opcodes.INVOKESTATIC,
                                "uk/co/mysterymayhem/debugmod/Hooks",
                                "dataManagerDebugHook",
                                "(Lnet/minecraft/network/datasync/DataParameter;Ljava/lang/Object;)V", false));
                        patchSuccessful = true;
                        break outerfor;
                    }
                }
        ...
        if (!patchSuccessful) {
            throw new RuntimeException("[MystDebugMod] EntityDataManager patch unsuccessful");
        }
    ...
    private static byte[] logEntityBoatGetDamageTaken(byte[] bytes) {
        String targetMethodName = Plugin.runtimeDeobfEnabled ? "func_70271_g" : "getDamageTaken";

        FMLLog.info("[MystDebugMod] Logging EntityBoat::getDamageTaken");
        ...
        for (MethodNode methodNode : classNode.methods) {
            if (methodNode.name.equals(targetMethodName)) {
                printMethodToFMLLogger(methodNode);
                FMLLog.info("[MystDebugMod] Logged EntityBoat::getDamageTaken");
                return bytes;
            }
        }
